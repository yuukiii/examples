 ...
 *    Please refer to <a href='http://docs.confluent.io/3.0.1/quickstart.html#quickstart'>CP3.0.1
 *    QuickStart</a>.
 *
 ...
 * $ java -cp target/streams-examples-3.0.1-standalone.jar io.confluent.examples.streams.TopArticlesExampleDriver
 * }
 * </pre>
 */
public class TopArticlesLambdaExample {
  ...
  static KafkaStreams buildTopArticlesStream(final String bootstrapServers,
    ...
    final KTable<Windowed<GenericRecord>, Long> viewCounts = articleViews
        ...
        .countByKey(TimeWindows.of("PageViewCountStore", 60 * 60 * 1000L), avroSerde);
    ...
    final KTable<Windowed<String>, String> topViewCounts = allViewCounts
        .mapValues(queue -> {
          final PriorityQueue<GenericRecord> copy = new PriorityQueue<>(comparator);
          copy.addAll(queue);
          StringBuilder sb = new StringBuilder();
          for (int i = 0; i < topN; i++) {
            GenericRecord record = copy.poll();
            if (record == null) {
              break;
            }
